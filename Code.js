...
const DB_SHEET_NAME = "VB_DB"; // informational
// === add to SHEET_IDS ===
const SHEET_IDS = {
  Users: "Users",
  Classes: "Classes",
...
function ensureVisionSheet_() {
  ...
  if (!sh) {
    ...
    sh.getRange(1, 1, 1, 5).setValues([
      ["id", "userId", "classId", "text", "updatedAt"],
    ]);
    sh.setFrozenRows(1);
  }
}
...
function openAdmin() {
  ...
  const html = HtmlService.createHtmlOutputFromFile("Admin")
    .setTitle("Vision Builder — Admin")
    .setWidth(1000)
    .setHeight(720);
  SpreadsheetApp.getUi().showModalDialog(html, "Vision Builder — Admin");
}
...
function getMe_() {
  ...
  if (!email) {
    throw new Error(
      "No identity. Share the web app with specific emails or your Workspace domain."
    );
  }
  const users = readRows_(SHEET_IDS.Users);
  const me = users.find(
  ...
  if (!me) {
    throw new Error(`User not registered in Users sheet: ${email}`);
  }
  return me;
}
...
function sheet_(name) {
  if (!name) {
    throw new Error(
      "Missing sheet name (got undefined). Check SHEET_IDS usage."
    );
  }
...
function getOpenAIKey_() {
  ...
  if (!key) {
    throw new Error("OpenAI key missing. Set Script property OPENAI_API_KEY.");
  }
  return key;
}
...
function buildVisionPrompt_(safeName, values, mission) {
  ...
  const missionLine =
    ...
    (mission?.description
      ? ` — ${String(mission.description).slice(0, 160)}`
      : "");
...
function api_generateVision(payload) {
  ...
  const enrolled = readRows_(SHEET_IDS.Enrollments).some(
    (e) => e.classId === payload.classId && e.userId === me.id
  );
  if (!enrolled) {
