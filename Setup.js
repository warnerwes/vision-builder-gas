/** RoboTeamUp — quick sheet bootstrap + seed (copy-paste ready)
 *  How to use:
 *  1) Open your Google Sheet → Extensions → Apps Script
 *  2) Paste this file, save, Run → setupAndSeedRTU()
...
function testClassroomAccess() {
  try {
    ...
    if (msg.includes('insufficientPermissions') || msg.includes('403')) {
      Logger.log('Hint: Enable the Classroom API and add the classroom.courses.readonly scope. Domain admin may need to approve.');
    } else if (msg.includes('401')) {
      Logger.log('Hint: Re-authorize the script (Run ▶ then Accept), or verify the signed-in account.');
    }
...
function setupAndSeedRTU() {
  ...
  makeSheet(ss, SHEETS.Users,              ['id','email','displayName','role','under13']);
  ...
  makeSheet(ss, SHEETS.ValueSelections,    ['id','userId','classId','valueId','coinWeight']);
  makeSheet(ss, SHEETS.MissionSelections,  ['id','userId','classId','missionId']);
  ...
  const users = [
    row(ids,'user_wes',  SHEETS.Users, { email:'wesley.warner@pacificcoastacademy.org', displayName:'Mr. Warner', role:'TEACHER', under13:false }),
  ...
  const enrollments = [
    // Teacher
    row(ids,'en_wes_go',        SHEETS.Enrollments, { userId: ids.user_wes, classId: ids.class_go,        roleInClass:'TEACHER' }),
    row(ids,'en_wes_iq',        SHEETS.Enrollments, { userId: ids.user_wes, classId: ids.class_iq_comp,   roleInClass:'TEACHER' }),
    row(ids,'en_wes_launch',    SHEETS.Enrollments, { userId: ids.user_wes, classId: ids.class_launchpad, roleInClass:'TEACHER' }),
    row(ids,'en_wes_coding',    SHEETS.Enrollments, { userId: ids.user_wes, classId: ids.class_coding,    roleInClass:'TEACHER' }),
    // Students
  ...
  const classMissionMap = [
    ...
    { classId: ids.class_iq_comp,   slugs:['all_the_way','states','regional','in_house','learn_explore'] },
  ...
  classMissionMap.forEach(entry => {
    entry.slugs.forEach(slug => {
      ...
      row(ids, uid(), SHEETS.ClassMission, { classId: entry.classId, missionId: mission.id });
    });
  });
  ...
  row(ids, uid(), SHEETS.MissionSelections, { userId: ids.user_chen, classId: ids.class_go,        missionId: mBySlug('learn_explore').id });
  // Alex (IQ) → states
  row(ids, uid(), SHEETS.MissionSelections, { userId: ids.user_alex, classId: ids.class_iq_comp,   missionId: mBySlug('states').id });
  // Bri (Launchpad) → in_house
  row(ids, uid(), SHEETS.MissionSelections, { userId: ids.user_bri,  classId: ids.class_launchpad, missionId: mBySlug('in_house').id });
  // Dev (Coding) → action_platformer
  row(ids, uid(), SHEETS.MissionSelections, { userId: ids.user_dev,  classId: ids.class_coding,    missionId: mBySlug('action_platformer').id });

  // Teams (example: create one team per class and add members)
  const t_go  = row(ids,'team_go',  SHEETS.Teams, { classId: ids.class_go,        name:'GO Gears',         missionId: mBySlug('learn_explore').id });
  const t_iq  = row(ids,'team_iq',  SHEETS.Teams, { classId: ids.class_iq_comp,   name:'IQ Knights',       missionId: mBySlug('states').id });
  const t_lp  = row(ids,'team_lp',  SHEETS.Teams, { classId: ids.class_launchpad, name:'Launch Stars',     missionId: mBySlug('in_house').id });
  const t_cod = row(ids,'team_cod', SHEETS.Teams, { classId: ids.class_coding,    name:'Code Crafters',    missionId: mBySlug('action_platformer').id });

  // Members (teacher optional; mostly students)
  row(ids, uid(), SHEETS.TeamMembers, { teamId: t_go.id,  userId: ids.user_chen, role:'MEMBER' });
  row(ids, uid(), SHEETS.TeamMembers, { teamId: t_iq.id,  userId: ids.user_alex, role:'MEMBER' });
  row(ids, uid(), SHEETS.TeamMembers, { teamId: t_lp.id,  userId: ids.user_bri,  role:'MEMBER' });
  row(ids, uid(), SHEETS.TeamMembers, { teamId: t_cod.id, userId: ids.user_dev,  role:'MEMBER' });

  SpreadsheetApp.getUi().alert('RoboTeamUp: tabs created and test data seeded ✅');
}
