...
function openIdLookup() {
  const html = HtmlService.createHtmlOutputFromFile("Ids")
    .setTitle("Vision Builder — ID Lookup")
    .setWidth(420);
...
function buildIdLookupSheet() {
  ...
  const { classes, users, missions, teams, classNameById } =
    api_ids_bootstrap();
  ...
  r = writeSection_(
    ...
    r,
    "Classes",
    ["Name", "Type", "GoogleClassroomId", "ID"],
  ...
  r = writeSection_(
    ...
    r,
    "Teams",
    ["Team Name", "Class Name", "ID"],
    teams.map((t) => [
      t.name,
      classNameById[t.classId] || t.classId || "",
      t.id,
    ])
  );
...
function highlightIdColumns() {
  ...
  Object.values(SHEET_IDS).forEach((name) => {
    ...
    const col = j + 1;
    if (sh.getLastRow() >= 1) {
      sh.getRange(1, col, sh.getLastRow(), 1).setBackground(color);
    }
...
function clearIdHighlights() {
  Object.values(SHEET_IDS).forEach((name) => {
    ...
    const col = j + 1;
    if (sh.getLastRow() >= 1) {
      sh.getRange(1, col, sh.getLastRow(), 1).setBackground(null);
    }
...
function writeSection_(sh, startRow, title, headers, rows) {
  sh.getRange(startRow, 1, 1, 1)
    .setValue(title)
    .setFontWeight("bold")
    .setFontSize(12);
  sh.getRange(startRow + 1, 1, 1, headers.length)
    .setValues([headers])
    .setFontWeight("bold")
    .setBackground("#f1f3f4");
  if (rows.length) {
    sh.getRange(startRow + 2, 1, rows.length, headers.length).setValues(rows);
  }
