const BACKUP_FOLDER_NAME = "VB_DB_BACKUPS";
...
function backupNow() {
  ...
  const blob = Utilities.newBlob(
    ...
    "application/json",
    `rtu_backup_${new Date().toISOString()}.json`
  );
  folder.createFile(blob);
  SpreadsheetApp.getUi().alert(
    `Backup written to Drive folder: ${BACKUP_FOLDER_NAME}`
  );
}

function restoreFromFilePrompt() {
  SpreadsheetApp.getUi().alert(
    `Open Drive folder ${BACKUP_FOLDER_NAME}, pick a backup JSON, then run restoreFromFile(fileId).`
  );
}
